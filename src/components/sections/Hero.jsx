  ...
  return (
    <section id="top" className="border-b border-line bg-[#f7f9fc]">
      <div className="mx-auto w-[min(1180px,calc(100%-28px))] md:w-[min(1180px,calc(100%-40px))]">
        <div className="grid gap-4 py-8 md:py-12 lg:grid-cols-[minmax(0,1.02fr)_460px] lg:py-16">
          <div className="border border-line bg-white p-6 md:p-10 lg:p-12">
            ...
            <div className="mt-12 max-w-3xl">
              <p className="mb-5 text-sm font-bold tracking-[0.16em] text-brand-700">{company.englishName}</p>
            ...
            <ul className="mt-8 flex flex-wrap gap-2">
              {hero.highlights.map((item) => (
                <li key={item} className="rounded-full border border-line bg-surface px-4 py-2 text-sm font-bold text-brand-900">
                  {item}
                </li>
              ))}
            </ul>
          ...
          <aside className="grid gap-4">
            <div className="border border-line bg-white p-6 md:p-8">
              <p className="text-sm font-semibold text-brand-700">BRAND SIGNATURE</p>
              <div className="mt-5 flex justify-center">
                <LogoMark size="hero" className="max-w-full" />
              </div>
              <p className="mt-5 text-center text-sm font-semibold leading-7 text-muted">
                전산용품 · 사무용품 · 인쇄물 · 판촉물 · 산업용 잡자재
              </p>
            </div>

            <div className="border border-line bg-brand-900 text-white">
              <div className="border-b border-white/15 p-6 md:p-8">
                <div className="flex items-start justify-between gap-6">
                  <div>
                    ...
                    <strong className="mt-2 block text-2xl font-bold break-keep">담당자 확인용 핵심 정보</strong>
                  </div>
                  <LogoMark size="sm" tone="white" className="max-w-[96px]" />
                </div>
              </div>
              ...
              <div className="grid gap-3 border-t border-white/15 p-6 md:p-8">
                ...
                <div className="flex gap-3 rounded-lg bg-white/10 p-4 ring-1 ring-white/15">
                  ...
                  <p className="text-sm leading-6 text-white/75">필요한 품목명과 수량, 희망 납기를 알려주시면 공급 가능 여부부터 빠르게 확인해드립니다.</p>
                </div>
              </div>
            </div>
          </aside>
